...
'</header>';
        ...
        const headerRegex = /<header class="fixed top-0 w-full[^>]*>[\s\S]*?<\/header>/;
        if (headerRegex.test(content) && content.includes('Clash Of Civil Servants')) {
            content = content.replace(headerRegex, newHeader);
            fs.writeFileSync(file, content);
            console.log('Updated ' + file + ' with responsive level header (Title: ' + title + ')');
        } else {
            console.log('Could not find suitable header in ' + file);
        }
    }
}
