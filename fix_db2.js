...
// Find the position right before closeModal
const targetIndex = html.indexOf('        function closeModal() {');

if (targetIndex !== -1) {
    // Also remove the broken piece right before it:
    // }
    //     saveDB();
    // }
    // }
    
    // The broken code starts right after the '}' of renderMath(el) which is around:
    const startOfBroken = html.lastIndexOf('            }', targetIndex);
    const earlierStart = html.lastIndexOf('            }', startOfBroken - 1);
    const evenEarlierStart = html.lastIndexOf('            }', earlierStart - 1); // let's find the `saveDB()`
    
    const saveDbIndex = html.indexOf('                saveDB();', earlierStart);
    
    let injectionPoint = targetIndex;
    let textToRemove = 0;
    
    if (saveDbIndex !== -1 && saveDbIndex < targetIndex) {
        // We need to remove from somewhere around the broken saveDB() block
        // Actually, let's just replace from the `            }\r\n                saveDB();`
        const regex = / {12}\}\r?\n {16}saveDB\(\);\r?\n {12}\}\r?\n {8}\}\r?\n\r?\n/;
...
`;
    // Insert just before closeModal
    html = html.replace('        function closeModal() {', replacementCode + '        function closeModal() {');
    fs.writeFileSync('database-soal.html', html);
    console.log("Fixed part 2!");
} else {
    console.log("Could not find closeModal");
}
